...
<div id="homepage">
	<div id="island">
		...
		project, a
		<strong>
			100% syllabus-compliant Pseudocode implementation written in standard CPython, with extension
			features!
		</strong>.
		...
		<ul>
			<li>It supports the <strong>0478, 2211, and 0984</strong> syllabi, <em>NOT A-LEVEL!</em></li>
			...
			<li>
				Since beancode (the interpreter itself) is written in Python, we bundle a Python interpreter
				as well, meaning that <strong>you can code in Python too!</strong> Note that formatting and
				tracing <em>do not work</em> when editing Python.
			</li>
		</ul>
		...
		<ul>
			...
			<li>
				There is also a file browser, emulating a UNIX-style file system. Downloading and uploading
		...
		<p>
			The Pseudocode interpreter itself runs within Python (as mentioned before). When you type code
			into the code editor and run it, it sends the code you wrote to a background
		...
		<p>
			Your code is also stored locally in IndexedDB with IDBFS, which are fancy terms for; it is all
